...
# -----------------------------------------------------------------------------
functions = [
    {"name": "create_task", "description": "Crea una tarea nueva en Notion.", "parameters": {"type": "object", "properties": {"title": {"type": "string", "description": "El título de la tarea."}, "description": {"type": "string", "description": "Una descripción opcional para la tarea."}, "category": {"type": "string", "description": f"La categoría de la tarea. Debe ser una de: {', '.join(VALID_CATEGORIES)}"}, "due_date": {"type": "string", "description": "La fecha de entrega, ej. 'mañana', '31 de diciembre', '25/12/2024'."}}, "required": ["title", "category", "due_date"]}},
    {"name": "list_tasks", "description": "Recupera una lista de tareas, opcionalmente filtradas por categoría o estado.", "parameters": {"type": "object", "properties": {"category": {"type": "string", "description": f"Filtrar por categoría. Opciones: {', '.join(VALID_CATEGORIES)}"}, "status": {"type": "string", "enum": ["Por hacer", "En progreso", "Hecho"], "description": "Filtrar por estado."}}}},
    {"name": "update_task", "description": "Actualiza el estado de una tarea existente, identificada por su título o ID.", "parameters": {"type": "object", "properties": {"task_id": {"type": "string", "description": "El ID de la tarea a actualizar."}, "title": {"type": "string", "description": "El título de la tarea a actualizar."}, "status": {"type": "string", "enum": ["Por hacer", "En progreso", "Hecho"], "description": "El nuevo estado de la tarea."}}, "required": ["status"]}},
    {"name": "delete_task", "description": "Elimina (archiva) una tarea existente, identificada por su título o ID.", "parameters": {"type": "object", "properties": {"task_id": {"type": "string", "description": "El ID de la tarea a eliminar."}, "title": {"type": "string", "description": "El título de la tarea a eliminar."}}}}
]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        response = client.chat.completions.create(model="gpt-4", messages=history, functions=functions, function_call="auto")
...
async def run_telegram_bot():
    """Inicializa y ejecuta el bot de Telegram de forma explícita y robusta."""
    print("Iniciando el bot de Telegram en modo explícito...")
    ...
    # 2. Inicializar la aplicación (prepara todo pero no empieza a recibir updates)
    await application.initialize()
    
    # 3. Iniciar el polling en segundo plano (empieza a pedir updates a Telegram)
    await application.updater.start_polling()
    
    # 4. Iniciar la aplicación (empieza a procesar los updates que lleguen)
    await application.start()
    
    print("¡Bot iniciado y escuchando! El servicio se mantendrá activo.")
    
    # 5. Mantener el script corriendo hasta que se reciba una señal de parada (ej. SIGTERM de Render)
    await application.updater.idle()

def run_cli():
    print("🟣 Olivia iniciada en modo CLI. Escribe 'salir' para terminar.\n")
    # Lógica de CLI no implementada
    while True:
        user_input = input("Tú: ")
